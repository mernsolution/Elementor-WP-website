...
if(isset($_POST['action'])){
	...
	if($action == "edit") {
		...
		<div id="update-form">
			<form id="add-form" name="add-form">
				...
				<div class="col-md-3">
					<input type="button" class="button button-primary button-hero" id="save_category" name="save_category" value="<?php _e('Update Category', 'portfolio-filter-gallery'); ?>" onclick="return DoAction('update', '<?php echo esc_html($id); ?>');" />
				</div>
	...
	if($action == "delete_all_category") {
		$exp_ids = sanitize_text_field($_POST['id']);
		$ids = explode("," ,$exp_ids);	
		$all_category = get_option('awl_portfolio_filter_gallery_categories');
		if(is_array($all_category)) {
			foreach($ids as $id) {
